  ...
  return product ? (
    <Main className={`mx-auto p-5`}>
      <section className={`shadow rounded-md`}>
        <article
          className={`flex flex-col md:flex-row flex-wrap text-gray-900`}
        >
          <div className={`w-full md:w-2/5 relative`}>
            {product.discount ? (
              <span
                className={`absolute w-10 h-10 rounded-full flex justify-center items-center top-3 right-3 font-bold transform rotate-12 text-red-500 border-2 border-red-500`}
              >
                {product.discount}%
              </span>
            ) : null}
          ...
          <div className={`flex-1 p-5 flex flex-col`}>
            <header
              className={`flex items-center justify-between relative mb-5 md:mb-0`}
            >
              ...
              {product.offPrice !== product.price && (
                <span
                  className={`text-red-500 main absolute right-0  top-6 lg:top-10 main text-xl`}
                >
                  ${product.offPrice}
                </span>
              )}
            ...
            <button
              ...
              className={`py-2 mt-5 bg-red-400 text-white rounded-md title flex items-center justify-center`}
            >
              {" "}
              <span>
                {checkInCart(cart, product._id) ? "in cart" : "add to cart"}
              </span>{" "}
              <MdOutlineAddShoppingCart className={`ml-3 text-emerald-300`} />
            </button>
          </div>
        </article>
      </section>
